...
const bindEventHandlers = () => {
    ...
    for (let fullscreenToggle of document.querySelectorAll(config.selectors.fullscreenOpen)) {
        fullscreenToggle.addEventListener('click', event => {
			...
			let key = event.target.getAttribute(attrName) || event.target.parentElement.getAttribute(attrName) || event.target.parentElement.parentElement.getAttribute(attrName)
