...
var defaults = {
     ...
     * @type {String}
     */
    layoutIncludes: ['src/views/fabricator/partials/*'],

    /**
     * Pages to be inserted into a layout
     * @type {(String|Array)}
     */
    views: ['src/views/**/*', '!src/views/+(fabricator)/**'],
...
var parseMaterials = function () {
    ...
    files.forEach(function (file) {
        ...
        localData = _.omit(localData, 'colors');

        var noHighlight = false

        // generate colod badges if color values are set
        if (fileMatter.data.colors) {
            noHighlight = true
            fileMatter.content = '<div class="f-color-boxes">' + fileMatter.content
            var colors = fileMatter.data.colors;
            var keys = Object.keys(colors)
            keys.forEach(key => {
               var color = colors[key]
               fileMatter.content += `
                    <div class="f-color-box" data-clipboard-trigger data-clipboard-text="${color.value}">
                        <div class="f-color-box__preview" style="background-color:${color.value}">
                            <span>${color.value}</span>
                        </div>
                        <div class="f-color-box__info">
                            ${key}
            ...
            fileMatter.content = fileMatter.content + '</div>'
        }

        if (fileMatter.data.logos) {
            noHighlight = true
            fileMatter.content = '<div class="f-logo-boxes">' + fileMatter.content
            var logos = fileMatter.data.logos;
            var keys = Object.keys(logos)
            keys.forEach(key => {
               var logo = logos[key]
               fileMatter.content += `
                    <div class="f-logo-box">
                        <div class="f-logo-box__preview">
                            <img src="${logo.assetPath}">
                        </div>
                        <div class="f-logo-box__info">
                            <span>${logo.note}</span>
                            <pre><code>/dist/${logo.assetPath}</code></pre>
                        </div>
                        <a href="/${logo.assetPath}">
                            <svg class="f-icon">
                                <use xlink:href="#f-icon-download"" />
                            </svg>
                        </a>
                    </div>
               `
        ...
        if (noHighlight) {
            fileMatter.content = '<no-highlight>' + fileMatter.content + '</no-highlight>'
        }
    ...
    // console.log(assembly.materials['01-branding'].items['01-colors'])
...
var parseViews = function () {
    ...
    files.forEach(function (file) {
        ...
        fileData = _.omit(fileData, 'colors');
